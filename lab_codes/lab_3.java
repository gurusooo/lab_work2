...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numIterations; i++) {
            ...
            List<Point> points = generatePoints(N, pX, pY);
            long endDataPreparation = System.nanoTime();
            totalDataPreparationTime += (endDataPreparation - startDataPreparation);

            SegmentTreeProccess solve = new SegmentTreeProccess();
            solve.preprocessData(rectangles);

            long startCountRectangles = System.nanoTime();
            for (Point point : points) {
                int count = solve.getPointRectsCount(point);
            }
    ...
    public static List<Point> generatePoints(int N, int pX, int pY) {
